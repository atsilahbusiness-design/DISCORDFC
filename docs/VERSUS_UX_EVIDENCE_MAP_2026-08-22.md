...
| Pre-dashboard identity/setup | Dedicated Versus walkthrough shows country selection, crest/logo selection, and club-name setup before the Versus dashboard, but ownership semantics are not established. | `WALKTHROUGH_OBSERVED` / disputed interpretation | System-managed assigned-team UX; no user-facing club-creation flow or proprietary crest asset |
...
The Discord implementation now exposes corresponding navigation from Versus Home: `Registration`, `Market`, `Rewards`, `Schedule`, `Rankings`, and `Global Ranking`, in addition to Home, Next Battle, Lineup, Results, and Standings. Entry is now `/versus-profile` or the Versus Home button, which automatically assigns a system-managed team/competition; `/versus-join` remains a private-group fallback. Market is a read-only preview. Sponsor purchase, diamond spending, advanced Scout offer generation, and Saves/Tackles telemetry remain unimplemented rather than fabricated because their exact costs, effects, and persistence were not visible in the verified source.
...
The advanced scout and player-status boost are intentionally not represented as fully implemented mechanics until their exact public flow, costs, persistence semantics, and effect formulas are observed. The country/crest/name sequence is directly observed but its ownership semantics are unresolved, so it is not implemented as user-created club metadata. The safe replacement is a system-managed assignment abstraction. Showing a fake scout screen would look more similar superficially but would violate the evidence boundary and could corrupt Versus balance.
