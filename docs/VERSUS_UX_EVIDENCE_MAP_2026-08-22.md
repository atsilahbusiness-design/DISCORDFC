...
The Discord implementation now exposes corresponding navigation from Versus Home: `Registration`, `Market`, `Rewards`, `Schedule`, `Rankings`, and `Global Ranking`, in addition to Home, Next Battle, Lineup, Results, and Standings. Entry is now `/versus-matchmake`, which assigns a system-managed team/competition; `/versus-join` remains a private-group fallback. Market is a read-only preview. Sponsor purchase, diamond spending, advanced Scout offer generation, and Saves/Tackles telemetry remain unimplemented rather than fabricated because their exact costs, effects, and persistence were not visible in the verified source.
