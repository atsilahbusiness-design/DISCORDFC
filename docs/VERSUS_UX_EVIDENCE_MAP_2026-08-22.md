...
| First-time club identity | Dedicated Versus walkthrough shows country selection, crest/logo selection, and club-name setup before the Versus dashboard. | `WALKTHROUGH_OBSERVED` | `/versus-club` plus owner-bound Discord modal; symbolic crest key only, no proprietary artwork |
...
The Discord implementation now exposes corresponding navigation from Versus Home: `Registration`, `Market`, `Rewards`, `Schedule`, `Rankings`, and `Global Ranking`, in addition to Home, Next Battle, Lineup, Results, and Standings. First-time identity setup is available through `/versus-club` and the owner-bound `Versus Club Setup` modal before group enrollment. Market is a read-only preview. Sponsor purchase, diamond spending, advanced Scout offer generation, and Saves/Tackles telemetry remain unimplemented rather than fabricated because their exact costs, effects, and persistence were not visible in the verified source.
...
The advanced scout and player-status boost are intentionally not represented as fully implemented mechanics until their exact public flow, costs, persistence semantics, and effect formulas are observed. Country/crest/name identity is the exception: its screen sequence is directly observed, so only the safe pre-enrollment metadata flow is implemented. Showing a fake scout screen would look more similar superficially but would violate the evidence boundary and could corrupt Versus balance.
