...
export const commandDefinitions = [
  ...
  new SlashCommandBuilder().setName('versus-club').setDescription('Buat atau atur identitas club Versus sebelum join group').addStringOption((option) => option.setName('name').setDescription('Nama club 2–32 karakter').setRequired(true)).addIntegerOption((option) => option.setName('country').setDescription('Country code 0–9999').setRequired(true).setMinValue(0).setMaxValue(9999)).addStringOption((option) => option.setName('crest').setDescription('Symbolic crest key, contoh red-star').setRequired(true)),
  ...
  new SlashCommandBuilder().setName('versus-roster').setDescription('Lihat player ID, posisi, eligibility, battle aktif, dan deadline Versus'),
